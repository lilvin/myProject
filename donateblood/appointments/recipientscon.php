...
die("connection failed:".mysqli_connect_error());
 ...
 $mobile = $_POST['mobile'];
  $location = $_POST['location'];
   ...
   $blood = $_POST['blood'];
    $date = $_POST['date'];
	  if(!empty($ID)&& !empty($mobile)  && !empty($location) && !empty($hospital) && !empty($blood) && !empty($date)){
	   
	   $sql = "INSERT INTO recipientsappointments(ID,mobile,location,hospital,blood,date) VALUES ('$ID','$mobile','$location','$hospital','$blood','$date')";
...
$date = $_POST['date'];
  ...
  $query ="SELECT * FROM recipientsreg WHERE (ID='$ID' AND password='$password')";
 ...
 $query= "DELETE FROM recipientsappointments WHERE ID='$ID' AND date='$date'";
if ($query_run= mysqli_query($con,$query)){
