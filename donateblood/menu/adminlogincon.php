...
$username = $_POST['username'];
 ...
 $password_hash=md5($password);
  ...
  $query ="SELECT * FROM adminreg WHERE (email='$username' AND password='$password_hash')";
...
<table width="669" border="1" align="center">
  ...
  <tr>
    ...
    <td width="201"><p>
      <input name="ID" maxlength="8" type="text" id="ID" value="<?php echo $ID; ?>"/>
      </p>        </td>
 ...
 $password_hash=md5($password);
  ...
  if(!empty($ID) && !empty($firstname) && !empty($lastname) && !empty($email) && !empty($mobile) && !empty($empID) && !empty($password)){
	   if (filter_var($email, FILTER_VALIDATE_EMAIL)){
	   	   $query ="SELECT * FROM adminreg WHERE (ID='$ID' AND password='$password_hash')";
